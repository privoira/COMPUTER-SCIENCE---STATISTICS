...
# calcolo la frequenza start p dell'allele A
# 2*p*(1-p)=Aa_start
# x = (2+(4-4*2*Aa_start)**0.5)/4

if (2+(4-4*2*Aa[0])**0.5)/4 > 0:
    p_start = (2+(4-4*2*Aa[0])**0.5)/4
else:
    p_start = ((2+(4-4*2*Aa[0])**0.5)/4)*(-1)

AA.append(p_start**2)
# calcolo la frequenza start degli omozigoti dominanti [Freq(AA) = F9)] e la assegno, come primo elemento, al vettore AA
...
# calcolo la frequenza degli omozigoti dominanti (AA = B11) dopo la prima generazione
# di selezione: Freq(AA) = AA[1]
# calcolo la frequenza degli eterozigoti [Freq(Aa) = C11] dopo la prima generazione di 
# selezione: Freq(Aa) = Aa[1] 
# AA[2] prima della selezione
# Aa[2] prima della selezione
